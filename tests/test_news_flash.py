...
from anyway.parsers.news_flash import scrape_rss_html


def fetch_rss_walla(url):
    with open('tests/walla.xml', encoding='utf-8') as f:
        return f.read()


def fetch_html_walla(url):
    with open('tests/{}.html'.format(url.split('/')[-1]), encoding='utf-8') as f:
        return f.read()
...
def test_parse_walla():
    ...
    items_actual = list(scrape_rss_html.fetch_and_parse_recent_news_items('walla',
                                                                          fetch_rss=fetch_rss_walla,
                                                                          fetch_html=fetch_html_walla))
    assert items_actual == items_expected


def fetch_rss_ynet(url):
    with open('tests/ynet.xml', encoding='utf-8') as f:
        return f.read()


def fetch_html_ynet(url):
    with open('tests/' + url[-len('0,7340,L-5735178,00.html'):], encoding='utf-8') as f:
        return f.read()
...
def test_parse_ynet():
    ...
    items_actual = list(scrape_rss_html.fetch_and_parse_recent_news_items('ynet',
                                                                          fetch_rss=fetch_rss_ynet,
